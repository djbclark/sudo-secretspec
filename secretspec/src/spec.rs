...
///
/// A `Spec` may be loaded from `secretspec.toml` with [`Spec::load_from`] or
...
impl Spec {
    ...
    /// so use [`Spec::load_from`] when the document uses inheritance.
    pub fn from_toml(source: &str) -> Result<Self> {
            ...
            .is_some_and(|extends| !extends.is_empty())
        {
            return Err(SecretSpecError::InvalidSpec(
                "a TOML string cannot resolve `project.extends`; load it from a path with Spec::load_from"
                    .to_string(),
            ));
        }
...
impl SpecBuilder {
    ...
    pub fn build(self) -> Result<Spec> {
        ...
        Spec::from_config_document(self.config)
    }
}
...
mod tests {
    ...
    fn string_input_rejects_unresolvable_extends() {
        ...
        assert!(error.to_string().contains("Spec::load_from"));
    }
