...
`--adopt-existing` is not optional on a host that already carries a boundary, and an automation that omits it is handing the operator a destructive command. Before 0.19.1-sudo.16 the fresh-install path recreated the vault's runtime files, truncating `.env` to zero bytes and destroying every stored secret value; the guard that refused a fresh install onto an existing identity or vault ran only inside the `--dry-run` branch, so the pre-flight refused precisely what the live run went on to do. Treat a green `--dry-run` on an older boundary as saying nothing about the live path. Since .16 the guard applies to both, and the runtime files are created only when missing, with an existing or symlinked `secretspec.toml`/`.env` refused rather than overwritten. Recovery from a truncated vault is from backup: rollback snapshots preserve the vault rather than restoring it, so there is nothing in the snapshot to recover the values from.
