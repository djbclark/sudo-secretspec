...
- A ref reads the environment the profile names unless `?env=` pins one. Infisical answers a
  missing secret, folder, environment and project with the same 404. Starting in SecretSpec 0.20,
  if every requested secret gets that ambiguous response, SecretSpec checks the environment root
